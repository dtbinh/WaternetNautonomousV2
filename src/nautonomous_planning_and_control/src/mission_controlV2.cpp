...
#include <nautonomous_planning_and_control/Obstacle.h>
...
int main(int argc, char **argv)
{
	...
	/* Get obstacles */
	Obstacle* ob = new Obstacle(0,0,0,3,5);
	...
	while (ros::ok())
	{
		...
		final_state.x = 20;
		final_state.y = 0;
		double theta = atan2(final_state.y - starting_state.y , final_state.x - starting_state.x);

		std::cout << "There are " << Obstacles->size() << " obstacles" << std::endl;
		/* Check if obstacle is in the way */
		for (int i = 0; i < Obstacles->size(); i++)
		{
			for (int j = 0; j < sqrt(pow(final_state.y - starting_state.y,2) + pow(final_state.x - starting_state.x,2)) ; j+= 0.1)
			{
				double state_x = starting_state.x + j * cos(theta);
				double state_y = starting_state.y + j * sin(theta);
				float l = pow((state_x - Obstacles->at[i].getX()) / Obstacles->at[i].getA() , 2) + pow((state_y - Obstacles->at[i].getY()) / Obstacles->at[i].getB() , 2);
				if l < 1;
				{
					bool obstacle_found = true;
					int obstacle_found_int = i;
					std::cout << "Obstacle " << i << " is in the way" << std::endl;
					/* Initialize a route */
					obstacle.major_semiaxis = Obstacles->at[i].getA();
					obstacle.minor_semiaxis = Obstacles->at[i].getB();
				}
			}
		}
