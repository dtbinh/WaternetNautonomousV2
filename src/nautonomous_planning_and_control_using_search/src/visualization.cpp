...
void obstacle_cb(const nautonomous_mpc_msgs::Obstacles::ConstPtr& obstacle_msg)
{
	...
	for (int i = 0; i < obstacles.obstacles.size(); i++)
	{
		...
		obstacle_marker.pose.position.x = obstacles.obstacles[i].state.pose.position.x + cos(obstacles.obstacles[i].state.pose.position.z) * obstacles.obstacles[i].state.twist.linear.x;
		obstacle_marker.pose.position.y = obstacles.obstacles[i].state.pose.position.y + sin(obstacles.obstacles[i].state.pose.position.z) * obstacles.obstacles[i].state.twist.linear.x;
		obstacle_marker.pose.orientation = toQuaternion(0.0, 0.0, obstacles.obstacles[i].state.pose.position.z);
