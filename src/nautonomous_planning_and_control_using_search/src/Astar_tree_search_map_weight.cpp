...
void add_new_node()
{
	...
	new_cost = current_node->getCost() + step_size + weighted_map.data[(floor((temp_y)/resolution)-1) * map_width + floor((temp_x)/resolution)]/10;
...
void make_weighted_map()
{
	...
	for (int i = 0; i < map_width; i++)
	{
		...
		{
			temp_map.data[j * map_width + i] = (int)(map.data[(j-2) * map_width + i] + map.data[(j-1) * map_width + i] + map.data[j * map_width + i] + map.data[(j+1) * map_width + i] + map.data[(j+2) * map_width + i]) / 5;
		}
	}
	...
	for (int i = 1; i < map_width-1; i++)
	{
		for (int j = 0; j < map_height; j++)
		{
			weighted_map.data[j * map_width + i] = (int)(temp_map.data[j * map_width + i - 2] + temp_map.data[j * map_width + i - 1] + temp_map.data[j * map_width + i] + temp_map.data[j * map_width + i + 1] + temp_map.data[j * map_width + i + 2]) / 5;
		}
	}
...
int main (int argc, char** argv)
{
	...
	map_pub = 	nh.advertise<nav_msgs::OccupancyGrid>("/weighted_map",10);
