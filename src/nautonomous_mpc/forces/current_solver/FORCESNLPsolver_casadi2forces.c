...
extern void FORCESNLPsolver_model_50_sparsity(int i, int *nrow, int *ncol, const int **colind, const int **row);
                  ...
                  )
{
    ...
    FORCESNLPsolver_FLOAT c_sparse[7];
    FORCESNLPsolver_FLOAT nabla_c_sparse[33];
    ...
    out[1] = nabla_f_sparse;
	 ...
	 if (stage >= 49 && stage < 50)
	 {
		 ...
		 /* copy to dense */
		 if( nabla_f ){ FORCESNLPsolver_model_50_sparsity(3, &nrow, &ncol, &colind, &row); sparse2fullCopy(nrow, ncol, colind, row, nabla_f_sparse, nabla_f); }
		 if( h ){ FORCESNLPsolver_model_50_sparsity(4, &nrow, &ncol, &colind, &row); sparse2fullCopy(nrow, ncol, colind, row, h_sparse, h); }
		 if( nabla_h ){ FORCESNLPsolver_model_50_sparsity(5, &nrow, &ncol, &colind, &row); sparse2fullCopy(nrow, ncol, colind, row, nabla_h_sparse, nabla_h); }
