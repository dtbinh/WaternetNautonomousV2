...
int main (int argc, char** argv)
{
	...
	obstacle.twist.linear.x = 1;
	obstacle.pose.orientation.z = 1.57;
	...
	{	
		...
		if(iter == 50)
		{
			...
			obstacles.obstacles[0].pose.position.y = 30;
		}
		else if((iter >= 100) && (iter < 500))
		{
			...
			obstacles.obstacles[1].pose.orientation.z = fmax(0.25,obstacles.obstacles[1].pose.orientation.z - 0.01);
		}
		else if((iter >= 500) && (iter < 1000))
		{
			obstacles.obstacles[0].twist.linear.x = fmin(1.5,obstacles.obstacles[0].twist.linear.x + 0.01);
			obstacles.obstacles[0].pose.orientation.z = fmin(1.25,obstacles.obstacles[0].pose.orientation.z + 0.01);
